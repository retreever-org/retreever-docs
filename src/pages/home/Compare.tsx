  ...
  return (
    <section className="relative py-10 px-16 bg-(--dark-3) overflow-hidden border-t border border-(--dark-border)/20">
      ...
      >
        ...
        >
          Zero-config API documentation that discovers your entire Spring Boot ecosystem automatically
        </motion.p>
      </motion.div>
      ...
      <motion.div
        ...
        className="max-w-7xl mx-auto overflow-x-auto border border-(--dark-border) rounded-3xl shadow-lg shadow-black/20"
      >
        <motion.table
          initial={{ opacity: 0 }}
            ...
            {comparisonData.features.map((row) => (
              ...
              >
                ...
                {comparisonData.tools.map((tool) => {
                  ...
                  let pillClass =
                    "inline-flex items-center justify-center px-3 py-1.5 rounded-full text-sm font-mono";
                  ...
                  return (
                    <td key={tool.id} className="px-6 py-4 text-center text-sm opacity-95">
